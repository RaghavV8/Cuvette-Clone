...
const CompanyApplicants = () => {
  ...
  const fetchApplicants = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error("Error in fetching applicants:", data.error);
        fetchApplicants();
      }
    } catch (error) {
  ...
  const filteredApplicants = applicants.filter(({ user, job , status}) => {
    ...
    const matchesStatus = selectedStatus ? applicationStatus === selectedStatus : true;
  ...
  const handleApplicationStatusChange = (applicationId, newStatus) =>{
    setApplicants((prevApplicants)=>
      ...
      // Refetch applicants to sync with the latest database changes after a short delay
  setTimeout(fetchApplicants, 2000);
  };

  //Interval to Keep on Refreshing the Jobs within 5 seconds
  useEffect(()=>{
        ...
        <div className="flex">
          <aside className='w-64 bg-white border-r border-gray-200 h-screen'>
            <nav className='mt-5 px-2'>
              <Link to="/company/home" className="mt-1 group flex items-center px-2 py-2 text-base leading-6 font-medium rounded-md text-gray-600 hover:text-gray-900 hover:bg-gray-50" >
              ...
              <Link to="/company/home/applied" className="mt-1 group flex items-center px-2 py-2 text-base leading-6 font-medium rounded-md text-blue-600 bg-gray-100" >
          ...
          <main className="flex-1 p-10 ">
            ...
            <div className="mb-5 mt-5" >
              <input type="text"
                className='p-2 border border-gray-300 rounded w-full' placeholder='Search Jobs...'
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            {filteredApplicants.length > 0 ? (
              filteredApplicants.map(({ _id, user, job, status }) => (
                <JobApplicantCards key={_id}
                  applicationId={_id}
