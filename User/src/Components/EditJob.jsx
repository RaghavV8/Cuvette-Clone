...
const CreateJob = () => {
    ...
    //Fetch Job Details for Form from Database
    useEffect(() => {
        if (!jobId) {
        ...
        const fetchJobDetails = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setValue("salary", data.job.salary);
                    setValue("location", data.job.location);
                    setSelectedJobType(data.job.jobType);
                }
            } catch (error) {
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <Navbar />
            <div className='w-full justify-center text-center m-4 text-2xl'> Job Editing Form </div>
